      ...
      fetch(`${API}/api/categories/${categoryInfo.categoryId}/movies`)
        ...
        .then(categoryMovies => {
          ...
          if (filters.source) {
            ...
            const sourceKey = sourceMap[filters.source] || filters.source.toLowerCase();
            filtered = filtered.filter(m => m.source === sourceKey);
    ...
    Object.entries(filters).forEach(([key, value]) => {
      if (key === 'genre') {
        ...
        if (value && value !== 'Tất cả') {
          const sourceKey = sourceMap[value] || value.toLowerCase();
          params.append('source', sourceKey);
        }
      } else {
          ...
          <Grid container spacing={3} className="movies-list">
            {pagedMovies.map((movie, idx) => (
              <Grid
                ...
                alignItems="center"
              >
                <Link to={`/movies/${movie.slug || movie.sourceSlug || movie.title_url || movie.id}`} style={{ textDecoration: 'none' }}>
                  <div className="movies-card-imgbox">
                      <img
                        src={movie.poster_url || movie.poster || movie.posterUrl}
                      alt={movie.title}
                      className="movies-card-img"
                ...
                <div className="movies-card-content">
                  ...
                  {movie.source && (
                    <div className="movies-card-source" style={{
                      fontSize: '10px',
                      color: '#FFD600',
                      marginTop: '4px',
                      fontWeight: 600
