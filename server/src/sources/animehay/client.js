...
class AnimehayClient {
  ...
  async fetchWithRetry(url, options = {}, retries = 1) {
    try {
      ...
      if (!response.ok) {
        throw new SourceError(`Animehay API error: ${response.statusText}`, 'animehay', 'HTTP_ERROR', response.status);
      }
      ...
      throw new SourceError(error.message, 'animehay', 'NETWORK_ERROR', 500);
    }
  }
