...
// Lấy pool kết nối từ app.locals. Nếu DB không sẵn sàng, trả về wrapper an toàn
const safePool = require('./src/db/pool');
console.log('safePool loaded:', { type: typeof safePool, execute: typeof safePool.execute, query: typeof safePool.query });
function getDb(req) {
  const appDb = req?.app?.locals?.db;
  if (req && req.app) console.log('getDb - req.app.locals.db type:', typeof req.app.locals.db);
  const pool = appDb || safePool;

  if (!pool) {
    console.warn('getDb: no pool available', { appDb: !!appDb, safePool: !!safePool });
    return {
      execute: async () => [[], []],
      query: async () => [[], []]
    };
  ...
  if (typeof pool.execute !== 'function' || typeof pool.query !== 'function') {
    console.warn('getDb: pool missing execute/query', { execute: typeof pool.execute, query: typeof pool.query });
    return {
      execute: async () => [[], []],
      query: async () => [[], []]
    };
  ...
  // wrapper to avoid throwing errors when DB is unavailable
  return {
    execute: async (sql, params) => {
      try {
        return await pool.execute(sql, params);
      } catch (err) {
        console.warn('DB execute failed:', err.message);
        return [[], []];
      }
    },
...
// Debug route to inspect DB objects at request time
router.get('/debug-db', (req, res) => {
  try {
    const appDb = req?.app?.locals?.db;
    res.json({
      appDbType: typeof appDb,
      appDbKeys: appDb ? Object.keys(appDb) : [],
      safePoolType: typeof safePool,
      safePoolKeys: safePool ? Object.keys(safePool) : []
    });
  } catch (err) {
...
router.get('/countries', async (req, res) => {
  try {
    const db = getDb(req) || safePool;
    console.log('countries handler db', db, 'execute type', typeof db?.execute);
    if (!db || typeof db.execute !== 'function') return res.json([]);
...
router.get('/genres', async (req, res) => {
  try {
    const db = getDb(req) || safePool;
    const pool = (!db || typeof db.execute !== 'function') ? safePool : db;
    const [rows] = await pool.execute('SELECT * FROM genres ORDER BY name').catch(() => [[], []]);
    res.json(rows);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
router.get('/categories', async (req, res) => {
  try {
    const db = getDb(req);
    const pool = (!db || typeof db.execute !== 'function') ? safePool : db;
    const [rows] = await pool.execute('SELECT * FROM categories ORDER BY id DESC').catch(() => [[], []]);
