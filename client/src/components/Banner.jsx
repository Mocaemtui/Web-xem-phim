...
export default function Banner() {
  ...
  useEffect(() => {
    // Banner component is disabled - using source providers instead
    // Set empty banners to hide the component
    setBanners([]);
  }, []);
  ...
  if (!banner) {
    return null; // Hide banner component when no banners available
  }
      ...
      >
        ...
        <Stack direction="row" spacing={2} alignItems="center">
          <Button
            ...
            onClick={() => {
              const bannerRoute = banner.slug || banner.movieSlug || banner.sourceSlug || banner.title_url || banner.movieId || banner.movie_id || banner.id;
              navigate(`/watch/${bannerRoute}`);
            }}
          >
            Xem ngay
          </Button>
          ...
          <IconButton className="banner-btn-icon" onClick={() => {
            const bannerRoute = banner.slug || banner.movieSlug || banner.sourceSlug || banner.title_url || banner.movieId || banner.movie_id || banner.id;
            navigate(`/movies/${bannerRoute}`);
          }}>
            <InfoOutlinedIcon />
          </IconButton>
        </Stack>
