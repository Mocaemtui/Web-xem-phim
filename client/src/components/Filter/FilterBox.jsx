...
export default function FilterBox({
  ...
  const handleFilter = () => {
    ...
    onFilter({
      ...
      sort,
      source: source !== 'Tất cả' ? source : null
    });
  };
  ...
  return (
    <Box className="filter-box">
      ...
      {/* Nguồn */}
      <Stack direction="row" alignItems="center" spacing={2} mb={2} flexWrap="wrap" className="filter-row">
        <Typography className="filter-label">Nguồn:</Typography>
        <div className="filter-btn-group">
          {["Tất cả", "NguonC", "KKPhim", "Anime47", "AnimeHay"].map(s => (
            <Button key={s} size="small" variant={source === s ? 'contained' : 'text'}
