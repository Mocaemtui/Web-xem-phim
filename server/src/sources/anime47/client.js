...
class Anime47Client {
  ...
  async fetchWithRetry(url, options = {}, retries = 1) {
    try {
      ...
      if (!response.ok) {
        throw new SourceError(`Anime47 API error: ${response.statusText}`, 'anime47', 'HTTP_ERROR', response.status);
      }
      ...
      throw new SourceError(error.message, 'anime47', 'NETWORK_ERROR', 500);
    }
  }
